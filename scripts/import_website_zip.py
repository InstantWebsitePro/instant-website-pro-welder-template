...
class ImportReport:
    ...
    def to_markdown(self) -> str:
        ...
        if not self.errors:
            lines.append("- The package failed for an unknown reason. Ask program support to review the workflow log.")
        return "\n".join(lines) + "\n"
...
def inspect_zip(package: Path, policy: dict[str, Any], report: ImportReport) -> list[zipfile.ZipInfo]:
    ...
    if report.compressed_bytes > int(policy["max_compressed_bytes"]):
        raise ImportRejected(
            "package.too_large",
            f"The ZIP is {report.compressed_bytes / 1024 / 1024:.2f} MiB; the program limit is {int(policy['max_compressed_bytes']) / 1024 / 1024:.0f} MiB. Move videos or large downloads to R2/Stream and rebuild.",
            package.name,
        )
    ...
    with zipfile.ZipFile(package) as zf:
        for info in zf.infolist():
            ...
            if info.file_size > int(policy["max_single_file_bytes"]):
                raise ImportRejected("zip.single_file_size", "An individual file exceeds the program asset limit.", normalized)
            ...
            if ext_limit is not None and info.file_size > int(ext_limit):
                raise ImportRejected("zip.extension_size", "This file type exceeds its program size limit.", normalized)
            ...
            with zf.open(info, "r") as source, destination.open("wb") as output:
                ...
                while True:
                    ...
                    if copied > info.file_size:
                        raise ImportRejected("extract.size_mismatch", "Expanded bytes exceeded inspected metadata.", name)
                    output.write(chunk)
                if copied != info.file_size:
