#!/usr/bin/env python3
"""Permit starter-mode CI only when public/ exactly matches the reviewed V7 plumber shell."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if actual != expected:
        print("Starter public/ differs from the reviewed V7 plumber shell.", file=sys.stderr)
        print(f"expected={expected}", file=sys.stderr)
        print(f"actual={actual}", file=sys.stderr)
        print("Set stage to production and complete all production gates, or have the program creator review and deliberately update the starter digest.", file=sys.stderr)
        return 1
    print(f"Reviewed V7 plumber starter tree verified: {actual}")
    return 0
