...
PLACEHOLDER_PATTERNS = [
    ...
    ("placeholder.replace", re.compile(r"\b(?:replace\s+me|insert\s+here|your\s+contractor|contractor\s+name\s+here)\b", re.I)),
    ("placeholder.brackets", re.compile(r"\[(?:contractor|business|domain|phone|address|city|owner|insert)[^\]]*\]", re.I)),
...
PRIVATE_CONTENT_PATTERNS = [
    ...
    ("privacy.business_record", re.compile(r"STRUCTURED BUSINESS DATA|WEBSITE BUSINESS FACTS|WEBSITE SOURCE OF TRUTH", re.I)),
...
REQUIRED_MANIFEST_KEYS = {
    "schema_version", "site_name", "business_type", "contractor_profile", "public_domain", "package_version",
...
class PageParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        tag = tag.lower()
        a = self._attrs(attrs)
        self.inline_event_handlers.extend(key for key in a if re.fullmatch(r"on[a-z]+", key))
        if tag == "html":
...
def validate_manifest(
    ...
    if schema_path is None or not schema_path.is_file():
        report.error("manifest.schema_missing", "The protected 6.0 transport manifest schema is required.", schema_path or "infrastructure/site-manifest.schema.json")
    else:
        try:
    ...
    if manifest.get("business_type") not in {
        "contractor", "general-contractor", "plumber", "electrician", "hvac",
        "welder-metal-fabricator", "landscaping-lawn-care", "roofing", "painting",
        "handyman-maintenance", "concrete-masonry", "cleaning-restoration",
        "pest-control", "pool-spa-service", "moving-service", "other-contractor-service",
    }:
        report.error("manifest.business_type", "This starter repository expects the contractor business module.", "site-manifest.json")
    contractor_profile = manifest.get("contractor_profile")
    if production and isinstance(contractor_profile, dict):
        if not contractor_profile.get("trade_categories"):
            report.error("manifest.trade_categories", "Production contractor packages need at least one confirmed trade category.", "site-manifest.json")
        if contractor_profile.get("operating_model") == "not-confirmed":
            report.error("manifest.operating_model", "Confirm whether the business is service-area, fixed-location, or hybrid before publishing.", "site-manifest.json")
        if contractor_profile.get("address_publication") == "not-confirmed":
            report.error("manifest.address_publication", "Confirm whether the public site may show an address before publishing.", "site-manifest.json")
        lead_actions = contractor_profile.get("lead_actions")
        if not isinstance(lead_actions, list) or not ({"call", "request-estimate"} & set(lead_actions)):
            report.error("manifest.lead_actions", "Production contractor packages need a confirmed call or request-estimate route.", "site-manifest.json")
...
def validate_headers(
    ...
    for line in text.splitlines():
        ...
        if not line[0].isspace():
            scope = stripped
        elif stripped.startswith("!") and stripped[1:].strip().lower() in required:
            report.error("headers.detached", "Required security headers may not be detached on any path.", "_headers")
        elif ":" in stripped and stripped.split(":", 1)[0].lower() in required and scope != "/*":
            report.error("headers.scope", "Baseline security headers must apply to /*, not only selected files.", "_headers")
    ...
    directive_names = [item.strip().split()[0].lower() for item in matches[0].split(";") if item.strip()]
    if len(directive_names) != len(set(directive_names)):
        report.error("headers.csp_duplicate", "Duplicate CSP directives can make browser enforcement differ from validation.", "_headers")
    directives = parse_csp(matches[0])
    for override in ("script-src-elem", "script-src-attr", "style-src-elem", "style-src-attr"):
        if override in directives:
            report.error("headers.csp_override", f"Use the audited script-src/style-src policy; {override} overrides need an infrastructure review.", "_headers")
    required_directives = {
        "default-src": "'self'",
        "base-uri": "'self'",
        "form-action": "'self'",
    ...
    for directive in ("default-src", "base-uri", "form-action", "frame-ancestors", "object-src"):
        expected = required_directives[directive]
        if directives.get(directive) != [expected]:
            report.error("headers.csp_baseline", f"{directive} must use only {expected}.", "_headers")
    script_tokens = directives.get("script-src", [])
    if "'unsafe-inline'" in script_tokens or "'unsafe-hashes'" in script_tokens or "blob:" in script_tokens:
        report.error("headers.csp_inline_script", "Inline script exceptions, unsafe hashes and blob scripts are not permitted.", "_headers")
    ...
    for directive in sorted(network_directives):
        for token in directives.get(directive, []):
            supported = {"'self'", "'none'"}
            if directive in {"img-src", "font-src"}:
                supported.add("data:")
            if directive == "media-src":
                supported.add("blob:")
            if directive == "style-src":
                supported.add("'unsafe-inline'")
            if not token.startswith(("https://", "http://")) and token not in supported:
                report.error("headers.csp_unsupported_source", f"Use exact HTTPS hosts or the supported keyword policy in {directive}; found {token}.", "_headers")
            ...
            if token.startswith(("https://", "http://")):
                ...
                if host is None:
                    ...
                    report.error(
                        ...
                        f"Content-Security-Policy {directive} allows {host}, but the 6.0 transport manifest does not declare a feature that needs it.",
                        "_headers",
                    )
...
def validate_page(
    ...
    if p.inline_event_handlers:
        report.error("javascript.inline_event", "Move inline event handlers to the site's external JavaScript so the security policy can stay enabled.", rel)
    for script in (item for item in p.images if item.get("tag") == "script"):
        if not script.get("src") and script.get("type", "").lower() not in {"application/ld+json", "application/json"}:
            report.error("javascript.inline_script", "Move executable inline scripts to local external JavaScript.", rel)
        ...
        for _, raw in pattern.findall(text):
            ...
            if not url or url.startswith(("data:", "http://", "https://", "#")):
                if url.startswith("http://"):
                    ...
                    report.error("css.external_asset", "CSS-referenced public assets must be packaged locally under the program CSP.", rel)
                continue
...
def validate_file_inventory(
    ...
    for entry in entries:
        ...
        if size > 20 * 1024 * 1024:
            report.error("asset.too_large", "Individual website assets must remain below 20 MiB program limit.", rel)
            ...
            if tag == "img":
                ...
                for candidate in candidates:
                    if candidate.startswith(("http://", "https://")):
                        report.error("asset.external_image", "Public images must be optimized local files under the program CSP.", page.relpath)
...
def validate_icons(
    ...
    for rel, (expected_rel, mime_type, expected_sizes) in required.items():
        ...
        if item.get("rel") != expected_rel or item.get("mime_type") != mime_type or not isinstance(actual_sizes, list) or set(actual_sizes) != expected_sizes:
            report.error("icon.contract", f"Icon declaration does not match the 6.0 transport contract: {rel}", "site-manifest.json")
        if not (root / rel).is_file():
...
def validate_site(
    ...
    contractor_schema_types = {
        "LocalBusiness", "HomeAndConstructionBusiness", "GeneralContractor", "Plumber",
        "Electrician", "HVACBusiness", "RoofingContractor", "HousePainter", "Locksmith",
        "MovingCompany", "ProfessionalService",
    }
    if production and not (contractor_schema_types & all_types):
        report.error("schema.contractor", "Production contractor websites need an applicable LocalBusiness or contractor subtype in JSON-LD.")
