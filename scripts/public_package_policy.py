#!/usr/bin/env python3
"""Shared public-tree and ZIP path policy for the V7 publishing system.
...
def load_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("policy_version") != "6.0.0" or policy.get("package_filename") != "website.zip":
        raise _fail("policy.version", "Policy version and package filename must match the 6.0 transport contract.", str(path))
    integer_keys = (
...
def validate_public_name(name: str, policy: dict[str, Any]) -> None:
    ...
    if filename_lower in {str(v).casefold() for v in policy["forbidden_exact_names"]}:
        raise _fail("privacy.filename", "A private, program, or starter-control filename is not public output.", name)
    if any(
        re.sub(r"[-_]+", " ", str(value).casefold()) in semantic_lower
        for value in policy["forbidden_name_parts"]
    ):
        raise _fail("privacy.filename", "A filename looks like private or program-controlled material.", name)
